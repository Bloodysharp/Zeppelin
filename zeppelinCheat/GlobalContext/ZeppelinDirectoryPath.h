...
class zeppelinDirectoryPath {
public:
    zeppelinDirectoryPath() noexcept
    {
#if IS_WIN64()
        ...
        pathString = mem::makeUniqueForOverwrite<wchar_t[]>(ntPathPrefix.length() + appDataPath.length() + kPathSeparatorLength + build::kzeppelinDirectoryName.length() + kNullTerminatorLength);
        if (!pathString)
            return;
        ...
        std::ranges::copy(build::kzeppelinDirectoryName, pathString.get() + writeIndex);
        writeIndex += build::kzeppelinDirectoryName.length();
        ...
        constexpr auto kPathSeparatorLength{1};
        constexpr auto kNullTerminatorLength{1};
        pathString = mem::makeUniqueForOverwrite<char[]>(homePath.length() + kPathSeparatorLength + build::kzeppelinDirectoryName.length() + kNullTerminatorLength);
        if (!pathString)
            return;
