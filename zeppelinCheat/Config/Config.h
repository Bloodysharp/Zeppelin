...
private:
    ...
    void saveToFile() noexcept
    {
        ...
        if (!hookContext.zeppelinDirectoryPath().get() || !state().pathToConfigDirectory || !state().pathToConfigFile || !state().pathToConfigTempFile)
            return;

#if IS_WIN64()
        WindowsFileSystem::createDirectory(hookContext.zeppelinDirectoryPath().get());
...
#elif IS_LINUX()
        mkdir(hookContext.zeppelinDirectoryPath().get(), 0777);
    ...
    {
        if (!hookContext.zeppelinDirectoryPath().get())
            return;

        const std::basic_string_view zeppelinDirectoryPath{hookContext.zeppelinDirectoryPath().get(), WIN64_LINUX(utils::wcslen, std::strlen)(hookContext.zeppelinDirectoryPath().get())};
        constexpr auto kPathSeparatorLength{1};
        constexpr auto kNullTerminatorLength{1};
        const auto length = zeppelinDirectoryPath.length() + kPathSeparatorLength + build::kConfigDirectoryName.length() + kNullTerminatorLength;
        state().pathToConfigDirectory = mem::makeUniqueForOverwrite<platform::PathCharType[]>(length);
        if (!state().pathToConfigDirectory)
            return;
        std::size_t writeIndex{0};
        std::ranges::copy(zeppelinDirectoryPath, state().pathToConfigDirectory.get() + writeIndex);
        writeIndex += zeppelinDirectoryPath.length();
